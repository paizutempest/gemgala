...
const log = {
    ...
    error: (msg) => console.log(`${chalk.red('✖')} [${dayjs().format('HH:mm:ss')}] ${chalk.redBright(msg)}`),
    gemgala: (msg) => console.log(`${chalk.bgGreen.black(' GEMGALA ')} ${msg}`)
};
...
function displayBanner() {
    ...
    ██████╔╝███████╗███████╗██║         ╚██████╔╝╚██████╗███████╗██║  ██║██║ ╚████║
    ╚═════╝ ╚══════╝╚══════╝╚═╝          ╚═════╝  ╚═════╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝
    AUTO REGISTER GEMGALA - DEEP OCEAN FLOW
    Modified by Gemini | User-Agent Injected
    `));
}

// --- SILUMAN BROWSER POLLING OTP ---
async function waitEmailOtp(email) {
    const emailLink = `https://generator.email/${email}`;
    const startTime = Date.now();
    log.process(`Siluman memantau Inbox: ${email}`);
    ...
    try {
        ...
        while (Date.now() - startTime < 120000) {
            const otpValue = await page.evaluate(() => {
                ...
                if (bodyText.includes("Gemgala") || bodyText.includes("OTP")) {
            ...
            if (otpValue) {
                log.success(`OTP DITEMUKAN: ${chalk.white.bold(otpValue)}`);
                await browser.close();
                return otpValue;
            }
        ...
        log.error(`Siluman Error: ${e.message}`);
    } finally {
        await browser.close();
    }
    return null;
...
// --- MAIN ENGINE (SNIFF ADAPTER) ---
async function startRegister(referralUid, count) {
    for (let i = 1; i <= count; i++) {
        const dev = DEVICE_LIST[Math.floor(Math.random() * DEVICE_LIST.length)];
        const deviceId = `ffffffff-${Math.random().toString(16).slice(2, 6)}-${Math.random().toString(16).slice(2, 6)}-0000-0000${Math.random().toString(16).slice(2, 10)}`;
        const timestamp = Math.floor(Date.now() / 1000);
        
        // Buat Nama & Email Acak
        const chars = 'abcdefghijklmnopqrstuvwxyz';
        let randName = '';
        for (let j = 0; j < 8; j++) randName += chars.charAt(Math.floor(Math.random() * chars.length));
        const email = `${randName}${Math.floor(1000 + Math.random() * 9000)}@konterkulo.com`;

        log.process(`[Akun ${i}] Device: ${dev[1]} | Email: ${email}`);

        try {
            // STEP 1: Kunci Referral (getPosition) - Data Sniff[cite: 1]
            log.process("Inject Referral Position...");
            await axios.post('https://api.igemgala.com/pub/preRegister/getPosition', 
                { uid: referralUid, os: "mobile" },
                { headers: { 'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36' } }
            );

            // STEP 2: Send Email OTP - Data Sniff[cite: 1]
            log.process("Meminta Kode OTP ke Gemgala...");
            const sendEmailRes = await axios.post('https://api.igemgala.com/pub/user/sendEmail', {
                email_address: email,
                TIMESTAMP: timestamp,
                device: deviceId,
                deviceName: dev[1],
            ...
            if (sendEmailRes.data.code !== 200) throw new Error(sendEmailRes.data.message);

            // STEP 3: Polling OTP via Siluman
            const otpCode = await waitEmailOtp(email);
            if (!otpCode) throw new Error("OTP Timeout!");

            // STEP 4: Register Akun - Data Sniff[cite: 1]
            log.process("Eksekusi Register Akun Baru...");
            const regRes = await axios.post('https://api.igemgala.com/pub/user/register', {
                email: email,
                email_code: otpCode,
                birthday: "2000-11-23", // Tanggal lahir kamu[cite: 1]
                sex: 1,
                device: deviceId,
                deviceName: dev[1],
            ...
            if (regRes.data.code === 200) {
                log.gemgala(chalk.greenBright(`REGISTRASI SUKSES! UID: ${regRes.data.data.user_id}`));
                
                // STEP 5: Report Game Duration (Biar Reff Masuk) - Data Sniff[cite: 1]
                log.process("Suntik Durasi Main (30s)...");
                await axios.post('https://api.igemgala.com/luckyBlock/standalone/reportGameDuration', {
                    gameDuration: 30,
                    user_id: regRes.data.data.user_id,
                    device: deviceId,
                    TIMESTAMP: timestamp + 60
                });
                log.success("Status: High Quality Invite Sent!");
            } else {
                log.error(`Gagal: ${regRes.data.message}`);
            }

       } catch (err) {
    // Ganti baris log.error yang lama dengan ini:
    log.error(`Proses Berhenti: ${err.response?.data?.message || err.message}`);
...
(async () => {
    ...
    const mode = await select({
        message: 'Deep Ocean Command Center:',
        choices: [
            { name: '🚢 Mulai Ternak Referral', value: 'run' },
            { name: '❌ Keluar', value: 'exit' },
        ],
    });

    if (mode === 'run') {
        const uid = await input({ message: "Masukkan UID Referral Kamu (Contoh: 26858630):", default: '26858630' });
        const jml = await input({ message: "Berapa banyak akun reff?", default: '1' });
        await startRegister(uid, parseInt(jml));
    }
})();